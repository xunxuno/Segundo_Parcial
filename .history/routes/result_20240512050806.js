...
router.get('/', (req, res) => {
  palabraController.procesarDatos(req, res, () => {
    if (req.session.palabraCache && req.session.palabraCache.textoOriginal && req.session.palabraCache.resultado && req.session.palabraCache.origen && req.session.palabraCache.destino) {
      // Renderizar la vista 'result' y pasar los datos necesarios
      console.log('renderizado exitoso');
      res.render('result', { title: 'Result', palabraCache: req.session.palabraCache });
    } else {
      ...
      console.log('Contenido de palabraCache:', req.session.palabraCache);
      res.redirect('/');
    }
  });
});
