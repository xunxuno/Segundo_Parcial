...
const visitCounter = require('../middlewares/visitCounter');
const { authenticate } = require('../middlewares/authMiddleware');

// Define la ruta GET para /resultado con el middleware de contador de visitas y autenticación
router.get('/', visitCounter, authenticate, async (req, res) => {
  if (palabraCache && palabraCache.textoOriginal && palabraCache.resultado && palabraCache.origen && palabraCache.destino) {
    ...
    const id_usuario = await obtenerIDPorNombre(req.userId);  // Usa req.userId en lugar de req.user.nombre
...
router.post('/', visitCounter, (req, res, next) => {
