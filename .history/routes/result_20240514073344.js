...
router.get('/', async(req, res) => {
  if (palabraCache && palabraCache.textoOriginal && palabraCache.resultado && palabraCache.origen && palabraCache.destino) {
    ...
    if (req.user && req.user.nombre) {
      // Si hay un usuario autenticado, obtenemos su ID
      id_usuario = await obtenerIDPorNombre(req.user.nombre);
    } else {
      // Si no hay un usuario autenticado, asignamos un usuario temporal ("invitado")
      // Aquí puedes implementar la lógica para generar un ID único para el usuario invitado
      // Por ejemplo, podrías generar un ID aleatorio o utilizar un ID específico para usuarios invitados
      // En este caso, asignaremos un valor fijo solo como ejemplo
      id_usuario = 'usuario_invitado';}
